<template>
  <div class="section_container h-[calc(100vh-64px)] flex flex-col gap-8">
    ...
    <div class="flex-1 flex flex-col gap-1 mx-2">
      <span class="text-gray-400 text-2xl ml-4 italic">Expenses List</span>
      ...
      <div v-else class="card">
        ...
        >
          ...
          <Column header="Number of Expenses" style="width: 20%">
            <template #body="slotProps">
              <span>{{ slotProps.data.expenses.length }} items</span>
            </template>
          </Column>
...
const deleleData = async () => {
  const loadingInstance = ElLoading.service({
    lock: true,
    text: "Deleting Expense Data...",
  });
  if (!selectedData.value) return;
  const docRef = doc(db, "expenses", selectedData.value.id);
